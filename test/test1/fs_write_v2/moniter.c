...
unsigned long get_fs_space(char *path)
{
	struct statfs fsbuf;
...
struct dirsname *get_fs_dirs_by_percent(struct dirsname *dirsp, int thread_n, long file_size)
{
	...
	while (tmp != dirsp) {
		...
		if ((j = get_fs_stat(path)) > i && get_fs_stat(path) > START_RELEASE && get_fs_space(path) > (unsigned long)thread_n*file_size) {
			i = j;
			result = tmp;
		}	
		tmp = tmp->next;		
...
struct dirsname *get_fs_dirs_by_size(struct dirsname *dirsp, int thread_n, long file_size)
{
	...
	while (tmp != dirsp) {
		sprintf(path, "%s/%s", root_dir, tmp->name);
		if ((j = get_fs_space(path)) > i && get_fs_stat(path) > START_RELEASE && get_fs_space(path) > (unsigned long)thread_n*file_size) {
			i = j;
			result = tmp;
		}	
		tmp = tmp->next;		
...
struct dirsname *get_fs_dirs_default(struct dirsname *dirsp,struct dirsname *tmp_bef, int thread_n, long file_size)
{
	...
	do{
		...
		if ((get_fs_stat(path)) > START_RELEASE && get_fs_space(path) >  (unsigned long)thread_n*file_size ) {
			return result;
		}
		result = result->next;
	}while(result !=  tmp);
